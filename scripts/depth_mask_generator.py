...
def main(dumped_file_folder, output_folder_name, model_name, save_overlayed_mask):
    ...
    for seq in tqdm(sequences, desc="Processing sequences"):
        ...
        for rgb_image_file in tqdm(rgb_image_files, desc="  Processing images"):
            ...
            depth_mask = (depth_image_npy > 35).astype(int)
            
            # depth_image = mask2colormap(depth_mask, plt.get_cmap('cubehelix'))
            ...
            if save_overlayed_mask:
                ...
                # 保存带有轮廓的叠加图像
                output_image_path = os.path.join(output_folder_path, rgb_image_file)
                cv2.imwrite(output_image_path, overlayed_image)

            # save depth_mask in .npy format
            depth_mask_path = os.path.join(output_folder_path, os.path.splitext(rgb_image_file)[0] + '.npy')
...
if __name__ == "__main__":
    ...
    main(args.dumped_file_folder, args.output_folder_name, args.model_name, args.save_overlayed_mask)
